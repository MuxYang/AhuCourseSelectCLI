...
class AHUCourseSelector:
    ...
    def __init__(self, token: Optional[str] = None, student_id: Optional[str] = None, ui: Optional[ConsoleUI] = None):
        ...
        self.manual_turn_id = None  # 手动指定的批次ID
    ...
    def load_credentials(self, filepath: str = "credentials.json") -> bool:
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                # 加载turn_id（如果存在）
                if 'turn_id' in creds and creds['turn_id']:
                    self.turn_id = creds['turn_id']
                    self._success(f"已加载凭证: student_id={self.student_id}, turn_id={self.turn_id}")
                else:
    ...
    def save_credentials(self, filepath: str = "credentials.json"):
        ...
        # 如果有turn_id，也一并保存
        if self.turn_id:
            creds['turn_id'] = self.turn_id
        
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(creds, f, indent=2, ensure_ascii=False)
        self._success(f"凭证已保存到 {filepath}")
    
    def query_available_turns(self) -> Optional[List[Dict]]:
        """
        查询当前可用的选课批次列表 
        
        Returns:
            可用批次列表，每个包含 id, name 等信息
        """
        # 尝试多个可能的端点来获取可用turn列表
        endpoints = [
            f"{self.API_BASE}/student/course-select/available-turns",
            f"{self.API_BASE}/student/course-select/turns",
            f"{self.API_BASE}/student/turns",
        ]
            ...
            try:
                ...
                if data.get('result') == 0 and data.get('data'):
                    ...
                    if isinstance(turns, list) and len(turns) > 0:
                        self._info(f"发现 {len(turns)} 个可用选课批次")
                        return turns
            except Exception:
                continue
        
        self._warn("未能查询可用批次列表，将使用默认批次")
        return None

    def set_manual_turn_id(self, turn_id: int):
        """
        手动设置选课批次ID
        ...
        Args:
            turn_id: 要设置的批次ID
        """
        self.manual_turn_id = turn_id
        self._success(f"已手动设置批次ID: {turn_id}")

    def get_turn_info(self, turn_id: Optional[int] = None) -> Optional[Dict]:
        """
        获取当前选课批次信息
        
        Args:
            turn_id: 选课批次ID，如果为None则自动探测
        ...
        # 优先使用手动设置的turn_id
        if self.manual_turn_id is not None:
            turn_id = self.manual_turn_id
            self._info(f"使用手动设置的批次ID: {turn_id}")
        # 其次使用传入的turn_id参数
        elif turn_id is None:
            # 如果已从credentials中加载了turn_id，直接使用
            if self.turn_id:
                turn_id = self.turn_id
                self._info(f"使用加载的批次ID: {turn_id}")
            else:
                # 尝试获取可用批次列表
                available_turns = self.query_available_turns()
                if available_turns and len(available_turns) > 0:
                    # 优先使用第一个可用批次
                    turn_id = available_turns[0].get('id') or available_turns[0].get('turnId')
                    self._info(f"使用发现的批次: {turn_id}")
                else:
                    # 回退到默认值（从最新的HAR文件看是921）
                    turn_id = 921
                    self._warn(f"使用默认批次ID: {turn_id}")
        
        url = f"{self.API_BASE}/{self.student_id}/turn/{turn_id}/select"
    ...
    def rapid_select_course(self, lesson_id: int, target_time: datetime, 
        ...
        self._info(f"等待时间: {target_time.strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            try:
                while True:
                    ...
                    if current_dt > deadline:
                        print()
                        self._error(f"超时 (已发起 {attempt_counter} 次)")
...
def load_credentials_with_retry(client: AHUCourseSelector, skip_default: bool = False) -> bool:
    """
    加载凭证，如果失败则要求用户输入新的凭证文件路径
    
    Args:
        client: 选课客户端实例
        skip_default: 如果为True，跳过默认凭证加载，直接进入选择菜单
        
    Returns:
        是否成功加载
    """
    # 尝试默认路径（除非明确跳过）
    if not skip_default and client.load_credentials():
        return True
...
def ensure_valid_credentials(client: AHUCourseSelector) -> bool:
    ...
    while True:
        ...
        # 验证失败时跳过默认凭证加载，直接显示菜单要求用户重新选择
        if not load_credentials_with_retry(client, skip_default=True):
            return False
...
def main():
    ...
    # 2. 获取目标时间
    while True:
        ui.step("请输入时间 (格式: YYYYMMDDhhmmss)")
        ui.info("示例: 20251208153000 表示 2025年12月8日15时30分00秒")
        time_str = ui.question("时间")
    ...
    ui.step("配置选课批次")
    if client.turn_id:
        ui.info(f"已从凭证加载批次ID: {client.turn_id}")
        modify = ui.question("是否要修改批次ID? (y=修改/n=使用当前)").lower()
        if modify == 'y':
            manual_turn = ui.question("请输入新的批次ID")
            try:
                client.set_manual_turn_id(int(manual_turn))
            except ValueError:
                ui.error("批次ID必须是数字，将使用原值")
    else:
        ui.warn("未检测到批次ID，请手动输入")
        manual_turn = ui.question("请输入批次ID (或按Enter跳过)")
        if manual_turn:
            try:
                client.set_manual_turn_id(int(manual_turn))
            except ValueError:
                ui.error("批次ID必须是数字，将在选课时自动探测")
    ...
    try:
        for i, target in enumerate(lesson_targets, 1):
            ...
            ui.divider(f"尝试 [{i}/{len(lesson_targets)}] {course_name} (lessonId={lesson_id})")
            
            success = client.rapid_select_course(lesson_id, target_time, time_offset)
            
            if success:
                success_any = True
                ui.success(f"成功! 课程: {course_name}")
                try:
