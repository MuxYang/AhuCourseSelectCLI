...
(function () {
    ...
    const FLAG_KEY = 'ahucourseselect_collect_after_reload';
    ...
    function createButton() {
        if ($('#ahucourseselect-helper-btn')) {
            return;
        }
        const btn = document.createElement('button');
        btn.id = 'ahucourseselect-helper-btn';
    ...
    function extractTurnIdFromUrl() {
        // 从URL中提取turn ID，格式：/course-select/{studentId}/turn/{turnId}/select
        const urlMatch = window.location.href.match(/\/turn\/(\d+)\//i);
    ...
    function collectAndDownload() {
        ...
        const turnId = extractTurnIdFromUrl();
        ...
        if (!turnId) {
            alert('未从URL中提取到 turn ID。请确保已进入选课页面（/course-select/xxx/turn/xxx/select）。');
        }
    ...
    function showOverlay() {
        if (document.getElementById('ahucourseselect-overlay')) {
            return;
        }

        const overlay = document.createElement('div');
        overlay.id = 'ahucourseselect-overlay';
        ...
        desc.textContent = '文件包含 token、学生 ID 和 turn ID，请检查后放入 AhuCourseSelectCLI 根目录。';
    ...
    function removeOverlay() {
        const overlay = document.getElementById('ahucourseselect-overlay');
        if (overlay) {
            overlay.remove();
        }
    }
