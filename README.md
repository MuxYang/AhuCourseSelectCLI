...
## 获取 token、学生 ID 和轮次 ID（浏览器脚本）
工具脚本需要的 `credentials.json`（示例见仓库同名文件）包含 `token`、`student_id` 和 `turn_id` 三个字段。推荐用篡改猴脚本自动抓取：

### 获取 token 和学生 ID
1. 安装 Tampermonkey 浏览器插件（或任意兼容的用户脚本管理器）。
2. 在浏览器中新建脚本，粘贴本仓库的 `ahucusthelper.user.js` 内容后启用（或者在安装了插件后直接拖入浏览器页面）。
...
### 获取轮次 ID（turn_id）
轮次 ID 会**随着选课窗口时间变动而改变**（每个学期的不同选课阶段使用不同的轮次），需要从选课页面获取：

**方式一（推荐）：从选课页面自动获取**
1. 进入选课页面：`https://jw.ahu.edu.cn/course-selection/#/course-select/{学号}/turn/{轮次ID}/select`
   - 需要在同一浏览器中登录教务系统
2. 进入页面后，点击**右下角的"导出选课凭据"按钮**（同获取token时的按钮）
3. 脚本会从URL中自动提取 `turn_id` 并保存到 `credentials.json`

**方式二：手动指定**
- 编辑 `credentials.json` 手动添加 `turn_id` 字段
- 或在程序运行时根据提示手动输入轮次 ID
...
1. 访问项目地址：https://github.com/MuxYang/AhuCourseSelectCLI
2. 点击绿色的 `Code` 按钮
3. 选择 `Download ZIP`
4. 下载完成后解压到本地目录
...
```bash
git clone https://github.com/MuxYang/AhuCourseSelectCLI.git
cd AhuCourseSelectCLI
```
